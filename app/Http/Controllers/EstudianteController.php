    ...
    public function updateAprobacionCurso(Request $request, string $id)
    {
        ...
        $data = $request->validate([
            "estado_aprobacion_curso" =>
                "required|in:pendiente,aprobado,reprobado",
        ]);
        ...
        if ($data["estado_aprobacion_curso"] === "aprobado") {
            ...
            if ($faltas > 0) {
                return response()->json(
                    [
                        "message" =>
                            "No se puede aprobar: el estudiante no cumplió con toda la asistencia (faltó a {$faltas} de {$sesionesRealizadas->count()} " .
